...
const RentalBatchManage = () => {
  ...
  const handleCellEdited = (cell) => {
    ...
    if (rowElement) {
      setTimeout(() => {
        ...
        rowElement.classList.add('edited'); // styled-components에서 정의한 클래스
      }, 0);
    }
  };
  ...
  const columns = [
    ...
    { headerHozAlign: 'center', hozAlign: 'center', title: '계약시작일', field: 'CONTRACT_STARTDT', sorter: 'string', width: 100, editor: 'input', editable: true, cellEdited: handleCellEdited },
    { headerHozAlign: 'center', hozAlign: 'center', title: '계약종료일', field: 'CONTRACT_ENDDT', sorter: 'string', width: 100, editor: 'input', editable: true, cellEdited: handleCellEdited },
  ...
  const handleProductSelect = (rowData) => {
    ...
    const updatedData = data.map((item) =>
      item.ID === selectedRow.ID // ID로 행 식별
        ? {
            ...item,
    ...
    setTimeout(() => {
      ...
      // 1. 데이터 갱신
      table.setData(updatedData).then(() => {
  ...
  const handleUserSelect = (rowDataArray) => {
    if (!selectedRow || !tableInstance.current || !rowDataArray || rowDataArray.length === 0) {
      console.warn('Invalid rowData or selectedRow:', { rowDataArray, selectedRow });
      setShowUserPopup(false);
      setSelectedRow(null);
      return;
    }

    const rowData = rowDataArray[0]; // 첫 번째 선택된 데이터 사용
    const updatedData = data.map((item) =>
      item.ID === selectedRow.ID
        ? {
            ...item,
            SECTIONNM: rowData.SECTIONNM || '',
            ORGNM1: rowData.ORGNM1 || '',
            ORGNM2: rowData.ORGNM2 || '',
            ORGNM3: rowData.ORGNM3 || '',
            ORGNM4: rowData.ORGNM4 || '',
            EMPNO: rowData.EMPNO || '',
            EMPNM: rowData.EMPNM || '',
            edited: true,
          }
