...
import styles from './StandardFieldWorkStatisticsPopup.module.css';
import { fetchJsonData } from '../../../utils/dataUtils';
import standardFieldworkPopupData from '../../../data/standardfieldwork_popup_data.json';

const StandardFieldWorkStatisticsPopup = ({ show, onHide, workHours }) => {
  ...
  const [filters, setFilters] = useState(initialFilters([
    { id: "filterSelect", label: "", type: "select", options: [{ value: "", label: "선택" }, { value: "본부", label: "본부" }, { value: "센터/부", label: "센터/부" }, { value: "부", label: "부" }, { value: "팀", label: "팀" }, { value: "업무량(시간)", label: "업무량(시간)" }] },
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setData([{ "본부": "오류", "센터/부": "데이터 없음", "부": "", "팀": "", "업무량(시간)": 0 }]);
      } finally {
        setLoading(false);
      }
    };
    ...
    const initializeTable = async () => {
      ...
      try {
        tableInstance.current = createTable(tableRef.current, [
          { title: '본부', field: '본부', hozAlign: 'left', width: 150 },
...
export default StandardFieldWorkStatisticsPopup;