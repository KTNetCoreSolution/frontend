...
const CarTransferConfirmList = () => {
    ...
    areas: [
      {
        type: 'search',
        fields: [
          { id: 'startDate', type: 'startday', row: 1, label: '기간', labelVisible: true, placeholder: '시작일 선택', width: '100px', height: '30px', backgroundColor: '#ffffff', color: '#000000', enabled: false, defaultValue: startDate },
  ...
  const visibleColumns = ['applyTarget|N'];  
  ...
  const handleReqStatus = async (seq, reqStatus) => {
    let msg = "";
    ...
    if (confirm(msg + "하시겠습니까?")) {
      ...
      try {
        const params = {
          pREQSTASTUS: reqStatus,
          pSEQ: seq,
          pEMPNO: user.empNo
        };

        const response = await fetchData("car/transferReqStatusTran", params);

        if (!response.success) {
          errorMsgPopup(response.message || msg + " 중 오류가 발생했습니다.");
          return;
        } else {
  ...
  const cellFormatter = (cell) => {{
      ...
      if ((rowData.REQSTATUS === "R" && rowData.CONFIRMYN === "Y") || (rowData.REQSTATUS === "Y" && rowData.TAKEOVERYN === "Y")) {
        const confirmButton = document.createElement("button");
        confirmButton.className = `btn btn-sm btn-primary`;
        confirmButton.innerText = "승인";

        if (rowData.REQSTATUS === "R" && rowData.CONFIRMYN === "Y") {  
          confirmButton.onclick = () => handleReqStatus(rowData.SEQ, 'Y');
        } else if (rowData.REQSTATUS === "Y" && rowData.TAKEOVERYN === "Y") {  
          confirmButton.onclick = () => handleReqStatus(rowData.SEQ, 'T');
        }
        ...
        rejectButton.innerText = "반려";

        if (rowData.REQSTATUS === "R" && rowData.CONFIRMYN === "Y") {  
          rejectButton.onclick = () => handleReqStatus(rowData.SEQ, 'C');
        } else if (rowData.REQSTATUS === "Y" && rowData.TAKEOVERYN === "Y") {  
          rejectButton.onclick = () => handleReqStatus(rowData.SEQ, 'N');
        }

        div.appendChild(rejectButton);
      } else if (rowData.REQSTATUS === "R") {
        label = "이관요청중";
        span.innerText = label;
        div.appendChild(span);
      } else if (rowData.REQSTATUS === "C") {
        label = "요청반려";
        span.innerText = label;
        div.appendChild(span);
        ...
        span.innerText = label;
        div.appendChild(span);
      } else if (rowData.REQSTATUS === "N") {
        label = "인수거부";
        span.innerText = label;
        div.appendChild(span);
      } else if (rowData.REQSTATUS === "T") {
        label = "이관완료";
        span.innerText = label;
        div.appendChild(span);
      }
  ...
  // 테이블 컬럼 정의
  const columns = [
    { frozen: true, headerHozAlign: "center", hozAlign: "center", title: "결제", field: "applyTarget", sorter: "string", width: 120, formatter: (cell) => cellFormatter(cell)},
    ...
    { title: '차대번호', field: 'CARID', width: 155, headerHozAlign: 'center', hozAlign: 'center' },
    { title: '요청자사번', field: 'REQUEST_EMPNO', width: 100, headerHozAlign: 'center', hozAlign: 'center', visible: false },
    { title: '요청자명', field: 'REQUEST_EMPNM', width: 90, headerHozAlign: 'center', hozAlign: 'center' },
    {
      title: '이관부서', headerHozAlign: 'center', hozAlign: "center",
      columns: [
        { title: '조직', field: 'PREORG_GROUP', width: 100, headerHozAlign: 'center', hozAlign: 'center' },
        { title: '본부', field: 'PREORGNMLV1', width: 120, headerHozAlign: 'center', hozAlign: 'center' },
        { title: '설계부/운용센터', field: 'PREORGNMLV2', width: 120, headerHozAlign: 'center', hozAlign: 'center' },
        { title: '부', field: 'PREORGNMLV3', width: 120, headerHozAlign: 'center', hozAlign: 'center' },
        { title: '팀', field: 'PREORGNMLV4', width: 120, headerHozAlign: 'center', hozAlign: 'center' },
        { title: '승인일시', field: 'CONFIRMDT', width: 140, headerHozAlign: 'center', hozAlign: 'center' },
        { title: '승인자사번', field: 'CONFIRM_EMPNO', width: 100, headerHozAlign: 'center', hozAlign: 'center', visible: false },
        { title: '승인자명', field: 'CONFIRM_EMPNM', width: 90, headerHozAlign: 'center', hozAlign: 'center' },
      ]
    },
    {
      title: '인수부서', headerHozAlign: 'center', hozAlign: "center",
      columns: [
        ...
        { title: '승인일시', field: 'TAKEOVERDT', width: 140, headerHozAlign: 'center', hozAlign: 'center' },
        { title: '승인자사번', field: 'TAKEOVER_EMPNO', width: 100, headerHozAlign: 'center', hozAlign: 'center', visible: false },
        { title: '승인자명', field: 'TAKEOVER_EMPNM', width: 90, headerHozAlign: 'center', hozAlign: 'center' },
      ]
    },
  ];
  // 데이터 로드 함수
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const response = await fetchData("car/transferConfirmInfo", params);
      if (!response.success) {
  ...
  return (
    <div className={styles.container}>
      ...
      <TableSearch
        ...
        onDownloadExcel={() => {handleDownloadExcel2(tableInstance.current, tableStatus, '차량이관 승인관리.xlsx', visibleColumns)}}
        rowCount={rowCount}
        onEvent={handleDynamicEvent}
      />
...
export default CarTransferConfirmList;