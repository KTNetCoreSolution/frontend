...
const MobileDrivingLog = () => {
  ...
  const getCarLogInfo = async () => {
    ...
    try {
      ...
      if (!response.success) {
        ...
        const logDate =
          pGubun === "I" ? (response.data[0].LOGDATE <= todayDate ? todayDate : response.data[0].LOGDATE) : response.data[0].LOGDATE;
        const logStTime =
          pGubun === "I" ? (response.data[0].LOGDATE === todayDate ? response.data[0].LOGENTIME : "08:00") : response.data[0].LOGSTTIME;
        ...
        const driver =
          pGubun === "I" ? user?.empNm + "(" + user?.empNo + ")" : response.data[0].DRIVER + "(" + response.data[0].DRIVER_EMPNO + ")";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const params = {
        pCARID: logInfo.CARID,
        pEMPNO: user?.empNo,
        pDRIVEREMPNO: logInfo.DRIVER_EMPNO,
        pLOGDATE: logInfo.LOGDATE,
        pLOGSTTIME: logInfo.LOGSTTIME,
        pLOGENTIME: logInfo.LOGENTIME,
        pSTKM: logInfo.STKM,
        pENKM: logInfo.ENKM,
        pFUEL: logInfo.FUEL,
        pNOTE: logInfo.NOTE,
        ...
        pETC1: Etc1,
        pETC2: Etc2,
        pSAFETYNOTE: logInfo.SAFETYNOTE,
      };
  ...
  const handleMoveRecipt = () => {
    navigate("/mobile/MobileCarLogReceipt", {
      state: { gubun: gubun, carId: logInfo.CARID, logDate: logInfo.LOGDATE, logTime: logInfo.LOGSTTIME, empNo: logInfo.EMPNO },
    });
  };
      ...
      <div className="pageMain">
          ...
          <div className="formList02">
            ...
            <div className="d-flex gap-1">
              <input
                ...
                style={{ width: "110px" }}
                value={logInfo.LOGDATE}
                disabled={gubun === "I" ? "" : "disabled"}
                onChange={(e) => {
                  handleLogDate(e);
                }}
              />
              <div className="d-flex flex-row align-items-center">
                <select
                  ...
                  value={logInfo.LOGSTTIME}
                  disabled={gubun === "I" ? "" : "disabled"}
                  onChange={(e) => {
                    handleLogTime(e, "stTime");
                  }}
                >
                  {stTime.map((time, index) => (
                ...
                <select
                  id="enTime"
                  className={`form-select ${styles.formSelect}`}
                  value={logInfo.LOGENTIME}
                  disabled={modifyYn === "Y" ? "" : "disabled"}
                  onChange={(e) => {
                    handleLogTime(e, "enTime");
                  }}
                >
                  {enTime.map((time, index) => (
                    <option key={index} value={time}>
                      {time}
                    </option>
                  ))}
        ...
        <div className="formDivBox">
          ...
          <ul className="formListData">
            <li>
              ...
              <div className="safetyWrap">
                <label
                  className={`${styles.safetyLabel} ${isDamage ? "" : styles.bad}`}
                  onClick={() => handleSafetyCheck("Damage", !isDamage)}
                >
                  {isDamage ? "양호" : "불량"}
                </label>
            ...
            <li>
              ...
              <div className="safetyWrap">
                <label
                  className={`${styles.safetyLabel} ${isOilLeak ? "" : styles.bad}`}
                  onClick={() => handleSafetyCheck("OilLeak", !isOilLeak)}
                >
                  {isOilLeak ? "양호" : "불량"}
                </label>
                <div
                  className={`${styles.safetyBox} ${isOilLeak ? "" : styles.bad}`}
                  onClick={() => handleSafetyCheck("OilLeak", !isOilLeak)}
                >
            ...
            <li>
              ...
              <div className="safetyWrap">
                <label className={`${styles.safetyLabel} ${isTire ? "" : styles.bad}`} onClick={() => handleSafetyCheck("Tire", !isTire)}>
                  {isTire ? "양호" : "불량"}
                </label>
            ...
            <li>
              ...
              <div className="safetyWrap">
                <label
                  className={`${styles.safetyLabel} ${isLuggage ? "" : styles.bad}`}
                  onClick={() => handleSafetyCheck("Luggage", !isLuggage)}
                >
                  {isLuggage ? "양호" : "불량"}
                </label>
                <div
                  className={`${styles.safetyBox} ${isLuggage ? "" : styles.bad}`}
                  onClick={() => handleSafetyCheck("Luggage", !isLuggage)}
                >
            ...
            <li>
              ...
              <div className="safetyWrap">
                <label className={`${styles.safetyLabel} ${isEtc1 ? "" : styles.bad}`} onClick={() => handleSafetyCheck("Etc1", !isEtc1)}>
                  {isEtc1 ? "양호" : "불량"}
                </label>
            ...
            <li>
              ...
              <div className="safetyWrap">
                <label className={`${styles.safetyLabel} ${isEtc2 ? "" : styles.bad}`} onClick={() => handleSafetyCheck("Etc2", !isEtc2)}>
                  {isEtc2 ? "양호" : "불량"}
                </label>
          ...
          <div className="mt-3">
            <textarea
              ...
              rows="5"
              value={logInfo.SAFETYNOTE}
              maxLength={1500}
              placeholder="점검특이사항(차량불량사항이 있는 경우/특수문자 입력불가)"
              disabled={modifyYn === "Y" ? "" : "disabled"}
              onChange={(e) => {
                setLogInfo({ ...logInfo, SAFETYNOTE: e.target.value });
              }}
            />
          </div>
        </div>
          ...
          <ul className="formDataWrap">
            ...
            <li>
              ...
              <div className="formData d-flex gap-1">
                ...
                <button
                  type="button"
                  className={`btn btn-secondary ${styles.btn} flex-shrink-0`}
                  style={{ display: logInfo.GUBUN === "I" ? "block" : "none" }}
                  onClick={(e) => {
            ...
            <li>
              ...
              <div className="formData">
                <input
                  ...
                  style={{ width: "120px" }}
                  value={logInfo.STKM}
                  disabled={gubun === "I" ? (logInfo.PRESTKM === 0 ? "" : "disabled") : "disabled"}
                  onInput={(e) => {
                    handleMaxLength(e, 11);
                  }}
                  onChange={(e) => {
                    setLogInfo({ ...logInfo, STKM: e.target.value });
                  }}
                />
              </div>
            </li>
            <li>
              ...
              <div className="formData">
                <input
                  ...
                  onChange={(e) => {
                    setLogInfo({ ...logInfo, ENKM: e.target.value });
                  }}
                />
              </div>
            </li>
            ...
            <li>
              ...
              <div className="formData">
                <input
                  ...
                  onChange={(e) => {
                    setLogInfo({ ...logInfo, FUEL: e.target.value });
                  }}
                />
              </div>
            </li>
            <li>
              ...
              <div className="formData">
                <input
                  ...
                  value={logInfo.NOTE}
                  onInput={(e) => {
                    handleMaxLength(e, 1000);
                  }}
                  onChange={(e) => {
                    setLogInfo({ ...logInfo, NOTE: e.target.value });
                  }}
                />
              </div>
            </li>
        ...
        <div className="btnWrap flex-column">
          <button
            ...
            style={{ display: user?.empNo === logInfo.EMPNO ? "block" : "none" }}
          ...
          <button
            className="btn btn-primary"
            style={{ display: logInfo.DELYN === "Y" ? "block" : "none" }}
